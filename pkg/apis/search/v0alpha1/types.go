...
// leaf from =~ into !~.
//
// Matching is against the whole indexed term and is case-sensitive. Field may
// name a filterable keyword string field that keeps its original case, or one
// metadata label value as labels.<key>. The pattern is a portable RE2 subset:
// literals, character classes, grouping, alternation, and greedy repetition.
// The backend is the source of truth for what the subset admits; it rejects
// unsupported syntax, fields that do not preserve case, and patterns that
// expand to too many terms with a 400.
//
// +k8s:deepcopy-gen=true
