...
- `regex`: `field`, `pattern`, and optional `negate`, matching how a single Prometheus matcher works (`negate` is `!~`). The field can be a string field declaring `filter`, or one metadata label named as `labels.<key>`. A declared field with that exact name wins over the label form. The match is against the whole indexed term and case-sensitive, so it only works on keyword fields that keep their original case; a field indexed lowercased (such as `title`) is rejected. `pattern` is a portable RE2 subset (literals, character classes, grouping, alternation, greedy repetition); the backend rejects unsupported syntax, case-losing fields, and patterns that expand to too many terms (10,000 inspected or matched) with a 400. An empty pattern is rejected, because it would match only the empty string and quietly return nothing.
